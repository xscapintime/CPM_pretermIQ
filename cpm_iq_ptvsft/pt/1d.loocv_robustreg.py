"""
Robust regression for feature seleciton in CPM
for there are outliers in the data
"""
import os, glob
import numpy as np
import pandas as pd
...
##### robust regression, select by p-val ######
# paras for CPM
cpm_kwargs = {'p_thresh': 0.01, 'corr_type': 'robust', 'verbose': False} ## use spearman if the distribution is skewed
...
for behav in behav_data.columns[:1]:
    ...
    behav_obs_pred, all_masks, corr = cpm_wrapper_seed_pval(all_fc_data, behav_data, behav=behav, k=k, seed=202210, **cpm_kwargs)
