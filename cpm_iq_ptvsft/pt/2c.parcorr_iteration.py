...
for behav in behav_data.columns[:1]:
    ...
    for n in range(iters, iters+20):
        ...
        behav_obs_pred, all_masks, corr = cpm_wrapper_seed_part_pval(all_fc_data, behav_shuff, behav=behav, covar=covar, k=k, seed=202211, **cpm_kwargs)
...
print("--- %s seconds for 1 behav, select by partial spearman pval---" %(time.time() - start_time))