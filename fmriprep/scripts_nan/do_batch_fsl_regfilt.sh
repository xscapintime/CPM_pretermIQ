...
for id in `ls /data/project/BIPP/laila/AP_8yo_fmri_task/derivatives/fmriprep/ | grep html | grep sub- | grep -v EAP |sed "s/sub-//g"| sed "s/.html//g" ` # check all successful AP and BIPP
do
